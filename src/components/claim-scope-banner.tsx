...
const CLAIMS: Record<string, { label: string; description: string; color: string; icon: typeof ShieldCheck }> = {
  signoff: {
    label: "SIGNOFF",
...
export function ClaimScopeBanner({
  scope
}: {
  scope: "signoff" | "demo" | "live-smoke";
}) {
  ...
  return (
    <div className={cn(
      ...
      <div className="flex-1 flex items-baseline gap-2">
        <span className="font-bold tracking-widest whitespace-nowrap">{claim.label} CLAIM ACTIVE:</span>
        <span className="font-medium">{claim.description}</span>
      </div>
      <Badge variant="outline" className={cn("text-[9px] h-4 uppercase font-bold tracking-tight px-1.5 bg-background/50", claim.color)}>
        {scope}
      </Badge>
    </div>
  );
