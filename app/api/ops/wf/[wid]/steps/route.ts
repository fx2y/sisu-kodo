...
} from "@src/contracts/ops/steps.schema";
import { getWorkflowSteps } from "@src/server/ops-api";
...
export async function GET(_req: Request, { params }: Props) {
  try {
    ...
    const out = await getWorkflowSteps(workflow, payload.id);
    assertGetWorkflowStepsResponse(out);
    return NextResponse.json(out, { status: 200 });
  } catch (error: unknown) {
    return toOpsErrorResponse(error, "GET /api/ops/wf/:wid/steps");
  }
}
